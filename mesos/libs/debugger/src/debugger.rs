...
impl<'a> Debugger<'a> {
    ...
    pub fn attach_internal(pid: u32, attached: bool) -> Debugger<'a> {
        ...
        // 4-KiB align the context (Win7 compat)
        let tmp  = context_backing.as_mut_ptr();
        let tmp  = ((tmp as usize) + 0xfff) & !0xfff;
        let cptr = tmp as *mut CONTEXT;
        let context = unsafe { &mut *cptr };
    ...
    /// Kill the process via `TerminateProcess()`
                ...
                EXCEPTION_DEBUG_EVENT => {
                    ...
                    if exception.ExceptionRecord.ExceptionCode == 0x80000003 {
                        ...
                        if exception.ExceptionRecord.ExceptionCode ==
                            ...
                            mprint!(self, "Unhandled exception {:x}\n",
...
impl<'a> Drop for Debugger<'a> {
    fn drop(&mut self) {
        ...
        mprint!(self, "Detached from process {}\n", self.pid);
    }
}
